(function(c, k, n, l) {
    ...
    c.extend(d.prototype, {
        ...
        bindCanvas: function() {
            this.$element.prepend('<div class="scratch-card-overlay"></div><canvas class="canvas" width="' + this.options.width + '" height="' + this.options.height + '" style="position: absolute;top: 0;z-index: 10;"></canvas>');
            this.initCanvas()
        },
        initCanvas: function() {
            var a = this;
            ...
            a.$element.find("canvas").on("mousemove touchmove." + a._name, function(b) {
                if (a.isDrawing) {
                    ...
                    for (var m = a.distanceBetween(a.lastPoint, b), c = a.angleBetween(a.lastPoint, b), d, h, g = 0; g < m; g++)
                        d = a.lastPoint.x + Math.sin(c) * g - 25,
                        h = a.lastPoint.y + Math.cos(c) * g - 25,
                        a.options.ctx.globalCompositeOperation = "destination-out",
                        a.options.ctx.drawImage(f, d, h);
                    a.lastPoint = b;
            ...
            // Create and position the overlay layer
            a.$element.find('.scratch-card-overlay').css({
                position: 'absolute',
                top: 0,
                left: 0,
                width: '100%',
                ...
                zIndex: 20 // Ensure the overlay is above the canvas
            });
        },
        ...
        getFilledInPixels: function(a) {
            if (!a || 1 > a)
                a = 1;
            for (var b = this.options.ctx.getImageData(0, 0, this.options.canvas.width, this.options.canvas.height).data, f = b.length, c = f / a, e, d = e = 0; d < f; d += a)
                0 === parseInt(b[d]) && e++;
        ...
        hasTouch: function() {
            return "ontouchstart"in k || 0 < navigator.maxTouchPoints || 0 < navigator.msMaxTouchPoints
        },
